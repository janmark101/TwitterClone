...
class TestNoPermissionURLS(APITestCase):
    ...
    def test_get_users_iduser_url(self):
        response = self.client.get(f'/auth/users/1/',format='json')
        self.assertEqual(response.status_code,status.HTTP_401_UNAUTHORIZED)
        
    def test_get_followersfollowinguser_url(self):
        response = self.client.get('/auth/followersfollowinguser/',format='json')
        self.assertEqual(response.status_code,status.HTTP_401_UNAUTHORIZED)
        
    def test_post_changecustomname_url(self):
        response = self.client.post('/auth/changecustomname/',None,format='json')
        self.assertEqual(response.status_code,status.HTTP_401_UNAUTHORIZED)
        
    def test_post_followuser_iduser_url(self):
